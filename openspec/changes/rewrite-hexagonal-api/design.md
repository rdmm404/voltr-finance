...
**Intentional architecture exception:** the review recommendation to replace the low-level budget repository and transactor with cohesive adapter operations such as `CreateMonthlyFromTemplate`, `CreateLineWithCategories`, `UpdateLineWithCategories`, and `LoadReportSnapshot` is declined for this change. The application therefore continues to orchestrate transaction callbacks, locking, copying, join-table replacement, and report reads through its repository port. The concrete races are fixed below, but the boundary itself is retained as a KISS trade-off and must not be described as a resolved review finding. It can be revisited when a use case demonstrates that the broader application-owned unit of work is insufficient.
