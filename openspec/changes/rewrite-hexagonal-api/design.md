...
Budget persistence uses a higher-level application-owned repository port with cohesive operations: `FindMonthly`, `CreateMonthlyFromTemplate`, `CreateLineWithCategories`, `UpdateLineWithCategories`, `DeleteLine`, and `LoadReportSnapshot`. The Postgres adapter owns transaction creation, transaction-scoped sqlc queries, row locking, template copying, category resolution and join-table replacement, and repeatable-read report snapshots. The application service retains validation, ensure/conflict-recovery policy, decimal normalization, and report total assembly, but it does not receive transaction callbacks or persistence-mechanic methods.

Within those cohesive adapter operations, concurrency-sensitive mechanics remain authoritative in PostgreSQL. Automatic budget-line ordering locks the parent budget before reading the next order, category assignment relies on the existing unique constraint rather than a race-prone pre-check, and constraint violations are translated to safe application conflicts. Multi-query reports run in a repeatable-read, read-only transaction, while writes retain normal read-committed behavior so a writer waiting on a parent lock sees the preceding committed line.
